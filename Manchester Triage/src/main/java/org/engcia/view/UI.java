...
public class UI {
    ...
    public static boolean answer(String ev, String v) {
        ...
        Collection<Evidence> evidences = (Collection<Evidence>) Triage.KS.getObjects(new ClassObjectFilter(Evidence.class));
        ...
        if (questionFound) {
            if (evidence.getValue().compareTo(v) == 0) {
                Triage.agendaEventListener.addLhs(evidence);
                return true;
            } else {
                return false;
            }
        ...
        if (value.compareTo(v) == 0) {
            Triage.agendaEventListener.addLhs(e);
            return true;
        } else {
            return false;
        }
